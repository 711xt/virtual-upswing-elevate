...
export default function Industries() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-white/10 bg-brand-charcoal/70 py-16">
        ...
        <div className="flex w-max marquee">
          {loop.map((name, i) => (
            <div key={i} className="flex items-center gap-12 px-6">
              <span className="whitespace-nowrap font-display text-3xl font-light text-white/80 md:text-5xl">
                {name}
              </span>
              <span className="h-2 w-2 rounded-full bg-brand-teal shadow-[0_0_12px_var(--brand-teal)]" />
            </div>
          ))}
        </div>
      </div>
